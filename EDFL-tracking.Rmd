...
Alternatively, if we set a **goal to end the project on `r format.Date(targetEndDate, "%B %d, %Y")`**, for example, our production rate needs to be **`r prettyNum(round(monthly.target,digits = 0), big.mark = ",")` images per month**. Practically, achieving the required month-over-month average necessitates setting a higher monthly production target, recognizing we will periodically fail to hit the mark. <mark>**We recommend setting a 5000-page monthly target for books sent to our D20 studio, and reassessing in October 2024**</mark>.
