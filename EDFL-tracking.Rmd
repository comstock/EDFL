...
With `r prettyNum(remaining, big.mark = ",")` images remaining, and an average production rate of `r prettyNum(round(av,digits = 0), big.mark = ",")`, our current projected completion date is `r format.Date(projectedEndDate, "%B %d, %Y")`.

However, if we set a **goal to end the project on `r format.Date(targetEndDate, "%B %d, %Y")`, our production rate needs to be `r prettyNum(round(monthly.target,digits = 0), big.mark = ",")` per month**.
