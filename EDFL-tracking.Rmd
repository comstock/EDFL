...
At the time, an **estimated `r prettyNum(images.remaining.as.of.start.measure.date,big.mark=",")` page-images remained**, and our production rate would have had us **completing the project in 2034**.
...
Subsequently, **Imaging Services has completed `r batches.completed` batches between June 2023 and `r format.Date(max(summary.EDFL$Date),"%B %Y")`, with page-images totaling `r prettyNum(sum(summary.EDFL$Quantity),big.mark = ",")`--an average `r prettyNum(round(average.images.month,digits = 0), big.mark = ",")` per month**.
...
Alternatively, if we set a **goal of completing the project on `r format.Date(targetEndDate, "%B %d, %Y")`**, for example, our production rate would need to be **`r prettyNum(round(monthly.target,digits = 0), big.mark = ",")` images per month**; and in practice, ensuring we complete the project on-time would require setting a more ambitious target, recognizing we will periodically fail to hit any mark we set.

In this scenario, <mark>**Imaging Services would recommend Houghton deliver 4000--5000 pages monthly to *D20*, in addition to any batches it delivers to *D40*, and that we reassess our progress in six-or-so months**</mark>.
